...
$query = "SELECT validated FROM vulnerabilities WHERE id = 14";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'Désactiver') {
        $sql = "UPDATE vulnerabilities SET validated = FALSE WHERE id = 14";
        ...
        $sql = "UPDATE vulnerabilities SET validated = TRUE WHERE id = 14";
        if ($conn->query($sql) === TRUE) {
